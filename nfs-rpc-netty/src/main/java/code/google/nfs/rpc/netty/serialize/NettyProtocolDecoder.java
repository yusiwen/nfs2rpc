...
public class NettyProtocolDecoder extends ChannelInboundHandlerAdapter {
  ...
  /**
   * If set then only one message is decoded on each {@link #channelRead(ChannelHandlerContext, Object)} call. This may
   * be useful if you need to do some protocol upgrade and want to make sure nothing is mixed up.
   *
  ...
  /**
   * If {@code true} then only one message is decoded on each {@link #channelRead(ChannelHandlerContext, Object)} call.
   *
   * Default is {@code false} as this has performance impacts.
   */
   ...
   * need to rely on this value to write a decoder. Use it only when you must use it at your own risk. This method is a
  ...
  /**
   * Gets called after the {@link ByteToMessageDecoder} was removed from the actual context and it doesn't handle events
   * anymore.
   */
