  ...
  public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
    if (e.getMessage() instanceof List) {
      ...
      if (isDebugEnabled) {
        ...
        LOGGER.debug("receive response list from server: " + ctx.getChannel().getRemoteAddress() + ",list size is:"
            + responses.size());
      }
      ...
      if (isDebugEnabled) {
        // for performance trace
        LOGGER.debug("receive response list from server: " + ctx.getChannel().getRemoteAddress() + ",request is:"
