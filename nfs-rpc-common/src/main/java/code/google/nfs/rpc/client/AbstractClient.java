...
public abstract class AbstractClient implements Client {
  ...
  public Object invokeSync(String targetInstanceName, String methodName, String[] argTypes, Object[] args, int timeout,
    ...
    RequestWrapper wrapper = new RequestWrapper(targetInstanceName.getBytes(), methodName.getBytes(), argTypeBytes,
        args, timeout, codecType, protocolType);
    return invokeSyncIntern(wrapper);
  }
  ...
  private Object invokeSyncIntern(RequestWrapper wrapper) throws Exception {
    ...
    if (result == null) {
      String errorMsg = "receive response timeout(" + wrapper.getTimeout() + " ms),server is: " + getServerIP() + ":"
          + getServerPort() + " request id is:" + wrapper.getId();
