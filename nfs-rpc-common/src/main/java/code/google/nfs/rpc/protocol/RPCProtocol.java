 ...
 * Common RPC Protocol
 *
 * Protocol Header VERSION(1B): Protocol Version TYPE(1B): Protocol Type,so u can custom your protocol Request Protocol
 * VERSION(1B): TYPE(1B): request/response CODECTYPE(1B): serialize/deserialize type KEEPED(1B): KEEPED(1B): KEEPED(1B):
 * ID(4B): request id TIMEOUT(4B): request timeout TARGETINSTANCELEN(4B): target service name length METHODNAMELEN(4B):
 * method name length ARGSCOUNT(4B): method args count ARG1TYPELEN(4B): method arg1 type len ARG2TYPELEN(4B): method
 * arg2 type len ... ARG1LEN(4B): method arg1 len ARG2LEN(4B): method arg2 len ... TARGETINSTANCENAME METHODNAME
 * ARG1TYPENAME ARG2TYPENAME ... ARG1 ARG2 ...
 *
 * Protocol Header VERSION(1B): Protocol Version TYPE(1B): Protocol Type,so u can custom your protocol Response Protocol
 * VERSION(1B): TYPE(1B): request/response DATATYPE(1B): serialize/deserialize type KEEPED(1B): KEEPED(1B): KEEPED(1B):
 * ID(4B): request id BodyClassNameLen(4B): body className Len LENGTH(4B): body length BodyClassName BODY if need than
 * set
 *
  ...
  public ByteBufferWrapper encode(Object message, ByteBufferWrapper bytebufferWrapper) throws Exception {
    ...
    if (message instanceof RequestWrapper) {
      try {
        ...
        int capacity = ProtocolUtils.HEADER_LEN + REQUEST_HEADER_LEN + requestArgs.size() * 4 * 2
            + targetInstanceNameByte.length + methodNameByte.length + requestArgTypesLen + requestArgsLen;
  ...
  public Object decode(ByteBufferWrapper wrapper, Object errorObject, int... originPosArray) throws Exception {
    ...
    if (version == (byte) 1) {
      ...
      if (type == REQUEST) {
        ...
        RequestWrapper requestWrapper = new RequestWrapper(targetInstanceByte, methodNameByte, argTypes, args, timeout,
