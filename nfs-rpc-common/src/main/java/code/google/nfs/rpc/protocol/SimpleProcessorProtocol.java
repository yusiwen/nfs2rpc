...
/**
 * Protocol Header VERSION(1B): Protocol Version TYPE(1B): Protocol Type,so u can custom your protocol Simple Processor
 * Protocol VERSION(1B): TYPE(1B): request/response CODECTYPE(1B): serialize/deserialize type KEEPED(1B): KEEPED(1B):
 * KEEPED(1B): ID(4B): request id TIMEOUT(4B): request timeout BodyClassNameLen(4B): body className Len LENGTH(4B): body
 * length BodyClassName: if need than set BODY
 *
